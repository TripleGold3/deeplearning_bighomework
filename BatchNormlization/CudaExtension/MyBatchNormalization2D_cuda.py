...
import MyBatchNorm2D_cuda
class MyBatchNormalization2dFunction_cuda(Function):
    @staticmethod
    def forward(ctx, someclass,  X, train_flag, epsilon, gamma, beta, monmentum, moving_mean, moving_var):
        if train_flag:           
            output, batch_norm, batch_mean, batch_var, batch_std, moving_mean, moving_var = \
                MyBatchNorm2D_cuda.train_forward(X, gamma, beta, moving_mean, moving_var, monmentum, epsilon)
            ...
            [output] = MyBatchNorm2D_cuda.validation_forward(X, gamma, beta, moving_mean, moving_var, epsilon)
        return output
    ...
    def backward(ctx, grad_output):
        ...
        grad_input, grad_gamma, grad_beta = \
            MyBatchNorm2D_cuda.train_backward(grad_output, input, X_norm, batch_mean, batch_var, batch_sqrt_var, gamma, beta, epsilon)    
        return None, grad_input, None, None, grad_gamma, grad_beta, None, None, None 

class MyBatchNormalization2d_cuda(nn.Module):
    def __init__(self, num_features, epsilon=1e-5, momentum=0.1, dtype=None, device=None):
        super(MyBatchNormalization2d_cuda, self).__init__()
    ...
    def forward(self, X):
        output =  MyBatchNormalization2dFunction_cuda.apply(self if self.training else None, X, self.training, self.epsilon, self.gamma, self.beta, self.momentum, self.moving_mean, self.moving_var)
        return output
...
if __name__ == '__main__':
    ...
    # model
    model = MyBatchNormalization2d_cuda(3)
    model.to(device)
